...
uv run ./led-badge-11x44.py -B50 -s4,10,10 -m7,5,5 \
  "joy" \
  "$(transform right "./art/longcat-dark-anim.gif")" \
  "$(BACKGROUND=black LOOP_REPEAT=24 transform move-right-loop "./art/running-cat-2-cropped.gif")"
...
# d="$(mktemp -d)"; f="$d/out.png"; uv run ./from_gif.py "$HOME/Documents/ledtag/running-cat-2-cropped.gif" "$f"; open "$f"
